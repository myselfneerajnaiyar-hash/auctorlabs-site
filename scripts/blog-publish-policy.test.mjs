...
test("an unpersisted featured image blocks publication", () => { const draft=row({frontmatter:{...row().frontmatter,image:"/missing-featured.png"}});assert.ok(publicationTechnicalBlockers(draft,[]).some(message=>message.includes("Featured image is not persisted"))); });
test("a matching persisted featured image permits publication", () => { const draft=row({frontmatter:{...row().frontmatter,image:"https://cdn.example/featured.png"}}),assets=[{id:"asset-featured",image_key:"featured",asset_type:"featured",status:"generated",public_url:"https://cdn.example/featured.png"}];assert.deepEqual(publicationTechnicalBlockers(draft,assets),[]); });
