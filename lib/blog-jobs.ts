...
import { createGenerationBudget } from "./blog-brief-request.mjs";
import fs from "node:fs";
import path from "node:path";
import matter from "gray-matter";
import type { BlogAdmin } from "./blog-admin-auth";
import { stageGeneratedImages } from "./blog-image-review.mjs";
...
export async function runBlogJob(admin:BlogAdmin,job:{id:string;topic:string;audience:string;manualResearch?:boolean;research_context?:{manualResearch?:boolean}}){
 ...
 let activeStage:Stage="researching";
 const stageState={...(claimed.stages as Record<string,string>||{})};
 try{
  ...
  budget.assertActive();
  const result=await generateBlog({deadline:budget.deadline,signal:budget.signal,topic:job.topic,audience:job.audience,withImage:true,manualResearch,researchContext:research.briefContext,onProgress:async(stage:Stage,state:State,detail:Record<string,number>={})=>{budget.assertActive();activeStage=stage;stageState[stage]=state;const current=await getCmsJob(job.id);budget.assertActive();if(current?.status!=="processing")throw new Error("Generation job is no longer active. Retry the job.");await heartbeatCmsJob(job.id,{stages:{...stageState},progress:{stage,...detail}});}}) as {slug:string;filePath:string;briefPath?:string};
  budget.assertActive();
  ...
  const staged=stageGeneratedImages(parsed.content,parsed.data);
  let row=await upsertCmsArticle(admin,{slug:result.slug,status:"draft",title:String(parsed.data.title),description:String(parsed.data.description||""),content:staged.content,frontmatter:staged.data,brief});
 ...
 }catch(error){await updateCmsJob(job.id,{status:"failed",stages:{...stageState,[activeStage]:"failed"},failure_kind:classifyGenerationFailure(error),error:`${activeStage}: ${error instanceof Error?error.message:String(error)}`,completed_at:new Date().toISOString(),lease_expires_at:null});}
}
