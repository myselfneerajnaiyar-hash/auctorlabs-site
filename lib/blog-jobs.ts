...
export async function runBlogJob(admin:BlogAdmin,job:{id:string;topic:string;audience:string}){
 const claimed=await claimCmsJob(job.id);if(!claimed)return;
 try{
  const corpus=await getSeoCorpus(),research=await researchTopic(job.topic,{corpus,adminId:admin.id});
  try{await heartbeatCmsJob(job.id,{research_snapshot_ids:research.snapshotIds,research_context:research});}catch(error){console.warn("SEO evidence job columns are unavailable until the Phase 3A migration is applied.",error);}
  const result=await generateBlog({topic:job.topic,audience:job.audience,withImage:true,researchContext:research.briefContext,onProgress:async(stage:Stage,state:State,detail:Record<string,number>={})=>{const current=await getCmsJob(job.id),nextStages={...(current?.stages||{}),[stage]:state};await heartbeatCmsJob(job.id,{stages:nextStages,progress:{stage,...detail}});}}) as {slug:string;filePath:string;briefPath?:string};
...
export async function retryBlogJob(admin:BlogAdmin,id:string){const job=await getCmsJob(id);if(!canRetryGenerationJob(job))throw new Error("This job is not retryable or has reached its retry limit.");await runBlogJob(admin,{id:job.id,topic:job.topic,audience:job.audience});return getCmsJob(id);}
