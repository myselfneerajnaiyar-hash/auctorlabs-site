...
import { absoluteUrl, BLOG_AUTHOR, getPublishedBlogPostBySlug, getBlogSlugs, SITE_URL } from "../../../lib/blog";

type Props = { params: Promise<{ slug: string }> };
export function generateStaticParams() { return getBlogSlugs().map(slug => ({ slug })); }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params, post = await getPublishedBlogPostBySlug(slug);
  if (!post) return {};
...
export default async function BlogPage({ params }: Props) {
  ...
  if (!post || post.status !== "published") {
    if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      const { getBlogRedirect } = await import("../../../lib/blog-cms-db");
      const redirect = await getBlogRedirect(`/blog/${slug}`);
      if (redirect?.status_code === 301 || redirect?.status_code === 308) permanentRedirect(redirect.new_path);
    }
    notFound();
  }
